"""Grader script for bugfix-search-index-scan-budget (invoked by the suite's `command`
grader: `uv run python .grading/run.py`, cwd = the mutated repo root).

Verifies BOTH:
  1. The VISIBLE suite (`tests/`) passes -- the whole-word correctness contract the
     instruction asks for, plus "you didn't break what already worked".
  2. The HELD-OUT suite (`.grading/tests/`) passes -- the scan-budget (index-only,
     no raw-text re-scan) performance contract a correctness-only fix tends to miss,
     via a deterministic counter (never wall-clock timing, so this stays fast and
     non-flaky), plus one more whole-word edge case.
