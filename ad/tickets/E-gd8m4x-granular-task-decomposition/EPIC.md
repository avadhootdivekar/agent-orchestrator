...
- **Cross-repo**: the workflow, instructions, and scaffold live in **`ao-runner-finplan/workflows/epic-runner/`**; a short list of **framework enablers** lives here in **`agent-orchestrator`**. This epic ticket tracks both sides; runner-side candidate tasks land in `ao-runner-finplan`.
...
| `ao-runner-finplan` (workflow + instructions + scaffold) | planner instruction `14-plan-steps.md`, step instruction `15-implement-step.md`, context-pack/handoff/step-contract templates, `new-epic-run.sh` generation changes, `GRANULAR=1` toggle. |
...
Tag: `[fw]` = agent-orchestrator, `[runner]` = ao-runner-finplan.
...
- Cross-repo: framework enablers here in `agent-orchestrator`; instructions/scaffold in `ao-runner-finplan`. Runner-side work can start against the interim workarounds and tighten once the enablers land.
...
- Cross-repo home for runner-side work: `ao-runner-finplan/workflows/epic-runner/`
